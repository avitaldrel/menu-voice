  ...
  return (
    <section aria-label="Menu results">
      ...
      <div className="mb-6">
        ...
        {menu.menuType && (
          <p className="text-muted-foreground mt-1 capitalize">{menu.menuType} menu</p>
        )}
      ...
      {menu.warnings.length > 0 && (
        <div role="alert" className="mb-4 p-3 bg-warning-surface border border-warning-border rounded-lg">
          <p className="font-medium text-warning-text">Note:</p>
          <ul className="list-disc list-inside text-warning-text text-sm mt-1">
  ...
  return (
    <div className="border border-muted rounded-lg overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3 text-left font-semibold text-lg bg-muted hover:bg-muted/80 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-accent transition-colors min-h-[48px]"
      >
        <span>
          {category.name}
              ...
              {item.dietaryFlags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {item.dietaryFlags.map((flag, k) => (
                    <span
                      key={k}
                      className="text-xs px-2 py-0.5 bg-accent/15 text-accent rounded-full"
                    >
                      {flag}
                    </span>
                  ))}
