...
- [x] **DEPLOY-01**: Security headers (CSP, Permissions-Policy, X-Content-Type-Options, X-Frame-Options, Referrer-Policy) on all HTTP responses
- [x] **DEPLOY-02**: /api/menu/extract timeout resilience (adapted: Node.js serverless maxDuration=60s instead of Edge Runtime due to 504s)
- [x] **DEPLOY-03**: All API routes validate Content-Type header and reject malformed requests with 415
- [x] **DEPLOY-04**: All API routes log structured JSON (event, IP, timestamp) for usage/spend monitoring via Vercel Runtime Logs
- [x] **DEPLOY-05**: Production build succeeds within Vercel Hobby tier limits
- [x] **DEPLOY-06**: .env.example documents all required environment variables with setup instructions
- [x] **DEPLOY-07**: App is live at a public Vercel URL with full user flow working end-to-end
